...
def resolve_csv_path():
    env_path = os.getenv("FIELDS_CSV_PATH", "").strip()
    ...
    for p in candidates:
        if p and os.path.isfile(p):
            return p
    raise FileNotFoundError("Arquivo CSV de campos adicionais não encontrado. Defina FIELDS_CSV_PATH ou coloque em data/.")

FIELDS_CSV_PATH = resolve_csv_path()
...
def main():
    if not API_TOKEN or not DSN:
        print("ERRO: MOVIDESK_TOKEN e/ou NEON_DSN não definidos nos Secrets.", file=sys.stderr)
        sys.exit(2)
