...
def fetch_ids_resolved_only(since_dt):
    url = f"{API_BASE}/tickets"
    top = 500
    throttle = float(os.getenv("MOVIDESK_THROTTLE","0.25"))
    skip = 0
    filtro = "(baseStatus eq 'Resolved' or baseStatus eq 'Closed' or baseStatus eq 'Canceled') and lastUpdate ge " + _iso_z(since_dt - datetime.timedelta(days=1))
    ...
    with psycopg2.connect(NEON_DSN) as conn:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                (since_dt, now, len(ids), len(missing))
            )
            ...
            if missing:
                psycopg2.extras.execute_batch(
                    ...
                    [(run_id,t,tid) for t,tid in missing],
                    page_size=500
                )
        conn.commit()
