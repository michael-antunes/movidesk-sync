...
def ensure_schema():
    with conn() as c:
        with c.cursor() as cur:
            ...
            # tabela base (se já existir, os alters abaixo cuidam das novas colunas)
            cur.execute("""
            create table if not exists visualizacao_resolvidos.tickets_resolvidos(
              ...
              -- campos já usados anteriormente
              owner_id text,
              owner_name text,
            ...
            """)
            # garante colunas novas/antigas sem erro se já existirem
            cur.execute("""
            do $$
            begin
...
(ticket_id,status,last_resolved_at,last_closed_at,last_cancelled_at,last_update,
 ...
 -- responsável/equipe (novos)
 responsible_id,responsible_name,owner_team_id,owner_team_name,
 -- ainda populamos os antigos owner_*, para compatibilidade
 owner_id,owner_name,
...
def fetch_pages(since_iso):
    ...
    # >>> adiciona ownerTeam para termos o id/nome da equipe
    expand = "owner,ownerTeam,clients($expand=organization)"
...
def map_row(t):
    ...
    # equipe do responsável (ownerTeam)
    ...
    return {
        ...
        # compat com colunas antigas (preenchidas igual ao responsável)
        "owner_id": resp_id,
        "owner_name": resp_name,
        # organização
