...
def fetch_recent_ids(since_iso):
    ...
    ids = []
    while True:
        page = _req(url, {"token": API_TOKEN, "$select": "id,lastUpdate", "$filter": filtro, "$orderby": "lastUpdate asc", "$top": top, "$skip": skip}) or []
...
def main():
    ...
    with psycopg2.connect(NEON_DSN) as conn:
        ...
        missing_rows = []
        for t in ("tickets_resolvidos", "resolvidos_acoes", "detail_control"):
            with conn.cursor() as cur:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                (start, now, len(ids), len(missing_rows)),
            )
