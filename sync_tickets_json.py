...
def fetch_ticket_detail(ticket_id):
    url_list = "https://api.movidesk.com/public/v1/tickets"
    r = requests.get(url_list, params={"token": API_TOKEN, "id": ticket_id}, timeout=90)
...
def enrich_clients_org(ticket_obj):
    ...
    for c in clients:
        ...
        if isinstance(org, dict) and (org.get("businessName") or org.get("id")):
            out.append(c); continue
        need.append(str(c.get("id"))) if c.get("id") else None
        out.append(c)
    if need:
        ...
        for c in out:
            if not isinstance(c, dict):
                new_out.append(c); continue
            if isinstance(c.get("organization"), dict):
                new_out.append(c); continue
            pid = str(c.get("id")) if c.get("id") else None
