...
def ensure_schema():
    with conn() as c:
        with c.cursor() as cur:
            ...
            # tabela principal (mantemos responsible_* por compatibilidade com a view existente)
            cur.execute("""
            ...
            # garante colunas novas sem dropar as antigas
            cur.execute("""
            do $$
            begin
              if not exists (
              ...
              if not exists (
                ...
                where table_schema='visualizacao_resolvidos'
                  and table_name='tickets_resolvidos'
                  and column_name='owner_team_name'
              ) then
...
(ticket_id,status,last_resolved_at,last_closed_at,last_cancelled_at,last_update,
 ...
 owner_id,owner_name,owner_team_id,owner_team_name,
...
values (%(ticket_id)s,%(status)s,%(last_resolved_at)s,%(last_closed_at)s,%(last_cancelled_at)s,%(last_update)s,
        ...
        %(owner_id)s,%(owner_name)s,%(owner_team_id)s,%(owner_team_name)s,
...
def fetch_pages(since_iso):
    ...
    # NADA de expand em ownerTeam (causou 400). Buscamos por $select:
    select_fields = ",".join([
...
def map_row(t):
    ...
    # equipe (sem expand): id e nome direto do select
    owner_team_id   = t.get("ownerTeamId")
    owner_team_name = t.get("ownerTeam")

    # org via clients[0].organization (quando existir)
    org_id = None
    org_name = None
    ...
    return {
        ...
        # compat com view antiga (preenche com owner):
        "responsible_id": owner_id if (owner_id is None or str(owner_id).isdigit()) else None,
