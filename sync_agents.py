...
def fetch_person_detail(pid):
    ...
    r = get_with_retry(f"{BASE}/persons/{pid}", p)
    return r.json()

def extract_email(detail, fallback_user_name):
    ...
    return best or fallback_user_name or ""
...
def extract_time_squad(detail):
    ...
    for v in vals:
        try:
            if int(v.get("customFieldId")) == CUSTOM_FIELD_SQUAD_ID:
                val = v.get("value")
