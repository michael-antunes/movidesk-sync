...
def fetch_status_history(ticket_id):
    ...
    r = session.get(f"{API_URL}/tickets/statusHistory", params=params)
...
def save_history(events, ticket_id):
    ...
    for ev in events:
        rows.append((
            ...
            ev.get("secondsUtL", 0),
            ev.get("permanencyTimeFullTimeSeconds", 0),
            ev.get("changedBy"),
            ev.get("changedDate"),
            ev.get("agentName"),
            ev.get("teamName")
    ...
    INSERT INTO visualizacao_resolucao.resolucao_por_status
      (ticket_id, status, justificativa, seconds_utl, permanency_time_fulltime_seconds, changed_by, changed_date, agent_name, team_name)
    VALUES %s
