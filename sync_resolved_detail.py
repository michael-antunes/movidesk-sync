...
def ensure_schema():
    with conn() as c, c.cursor() as cur:
        ...
        """)
        # adiciona/garante as novas colunas
        cur.execute("""
        do $$
        begin
          if not exists(select 1 from information_schema.columns
            ...
            alter table visualizacao_resolvidos.tickets_resolvidos add column responsible_id text;
          end if;
          if not exists(select 1 from information_schema.columns
                        where table_schema='visualizacao_resolvidos'
                          and table_name='tickets_resolvidos'
                          and column_name='responsible_name') then
            alter table visualizacao_resolvidos.tickets_resolvidos add column responsible_name text;
          end if;
          if not exists(select 1 from information_schema.columns
                        where table_schema='visualizacao_resolvidos'
                          and table_name='tickets_resolvidos'
...
(ticket_id,status,last_resolved_at,last_closed_at,last_cancelled_at,last_update,
 ...
 responsible_id,responsible_name,owner_team_id,owner_team_name,
...
values (%(ticket_id)s,%(status)s,%(last_resolved_at)s,%(last_closed_at)s,%(last_cancelled_at)s,%(last_update)s,
        ...
        %(responsible_id)s,%(responsible_name)s,%(owner_team_id)s,%(owner_team_name)s,
...
def select_expand_that_works(since_iso, url, select_fields):
    """Testa expands possíveis e devolve o primeiro que funciona (HTTP 200)."""
    candidates = [
        "owner,ownerTeam,clients($expand=organization)",   # preferido
        "owner,clients($expand=organization)"              # fallback sem ownerTeam
    ]
    for exp in candidates:
        try:
            req(url, {
                ...
                "$expand": exp,
                "$filter": f"(baseStatus eq 'Resolved' or baseStatus eq 'Closed' or baseStatus eq 'Canceled') and lastUpdate ge {since_iso}",
            ...
            time.sleep(0.2)
            continue
    # se nenhum funcionou, propaga o último erro
    raise last  # noqa: F822
...
def fetch_pages(since_iso):
    ...
    # descobre qual expand funciona no tenant
    expand = select_expand_that_works(since_iso, url, select_fields)
...
def map_row(t):
    ...
    resp_id   = owner.get("id")
    resp_name = owner.get("businessName") or owner.get("fullName") or owner.get("name")

    team = (t.get("ownerTeam") or {})  # pode não existir quando usamos o fallback
    team_id   = team.get("id")
    team_name = team.get("name") or team.get("businessName")
    ...
    return {
        ...
        "owner_id": resp_id,           # compat
        "owner_name": resp_name,       # compat
