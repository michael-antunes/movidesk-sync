...
def upsert_detail_control_and_mark(conn, items):
    with conn.cursor() as cur:
        if items:
            rows = [(int(i["id"]), i.get("lastUpdate")) for i in items if str(i.get("id","")).isdigit()]
            if rows:
                psycopg2.extras.execute_batch(cur,
                    "insert into visualizacao_resolvidos.detail_control (ticket_id,last_update) values (%s,%s) "
...
def main():
    ...
    conn.commit()
    try:
        since = get_since_utc(conn)
        items = fetch_index(iso_z(since))
        upsert_detail_control_and_mark(conn, items)
        print(f"INDEX since={iso_z(since)} rows={len(items)}")
    finally:
        conn.close()
